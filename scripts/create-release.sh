...
GITHUB_REPO="catPaw-s/Vibe-Island"

# Website repo for auto-updating appcast
WEBSITE_DIR="${CLAUDE_ISLAND_WEBSITE:-$PROJECT_DIR/../VibeIsland-website}"
WEBSITE_PUBLIC="$WEBSITE_DIR/public"

APP_PATH="$EXPORT_PATH/Vibe Island.app"
APP_NAME="VibeIsland"
...
if command -v create-dmg &> /dev/null; then
    ...
    create-dmg \
        ...
        --window-size 600 400 \
        --icon-size 100 \
        --icon "Vibe Island.app" 150 200 \
        --app-drop-link 450 200 \
        --hide-extension "Vibe Island.app" \
        "$DMG_PATH" \
        "$APP_PATH"
else
    echo "Using hdiutil (install create-dmg for prettier DMG: brew install create-dmg)"
    hdiutil create -volname "Vibe Island" \
        -srcfolder "$APP_PATH" \
        -ov -format UDZO \
        "$DMG_PATH"
fi
